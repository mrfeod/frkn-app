...
# Conditionally include --sign flag for installer packaging
SIGN_INSTALLER_ARGS=()
if [ -n "${MAC_INSTALLER_SIGNER_ID-}" ]; then
  SIGN_INSTALLER_ARGS=(--sign "$MAC_INSTALLER_SIGNER_ID")
fi
...
if [ -n "${MAC_INSTALL_CERT_PW-}" ] && [ "${NOTARIZE_APP+x}" ]; then
